...
define("DELEGATE_URL1", 'http://romantictravel.com.au');

//define("DELEGATE_URL1", 'http://localhost/worldtravel');
	...
	if(is_array($result) && $result["valid"] == 1){
		...
		if($result["post_id"] != 0){
			...
			$content['custom_fields'] = array( 
					...
					array('id' => $post_meta_hash['package_discount']['meta_id'], 'key' => 'package_discount', 'value' => $_POST['list_package_discount']),
			...
			$content['custom_fields'] = array(
					...
					array('key' => 'package_discount', 'value' => $_POST['list_package_discount']),
...
function save_package_remote( $post_id, $post, $update ) {
	...
	if($post->post_status == "publish"){
		edit_remote_site_travel_package($post, constant("DELEGATE_URL1"));
		return;
	}
	
	if($post->post_status == "trash"){
		delete_remote_site_travel_package($post, constant("DELEGATE_URL1"));
		return;
	}
...
function add_attachment_hook( $attach_id ) {
	...
	$client = new IXR_Client(constant("DELEGATE_URL1") . '/xmlrpc.php');
...
//add_filter( 'json_authentication_errors', 'simple_authentication', 10);
